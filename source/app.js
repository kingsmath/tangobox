...
/* ---- 랜덤 모드 (세트·코르티나 없이 곡만 무작위로 뽑는다) ---- */
const RANDOM_MODES = {
  'rnd-all':     ['tango','vals','milonga'],
  'rnd-tango':   ['tango'],
  'rnd-vals':    ['vals'],
  'rnd-milonga': ['milonga']
};
const isRandomMode = mode => !!RANDOM_MODES[mode];
...
/** 해당 장르(들)의 모든 곡을 {tanda, song} 으로 펼친다. */
function songPool(genres){
  const pool = [];
  genres.forEach(g => (DATA[g] || []).forEach(t => t.songs.forEach(s => pool.push({tanda:t, song:s}))));
  return pool;
}
/** 곡 하나짜리 슬롯 (딴따 구조를 그대로 쓰되 count 를 1로 둔다) */
const mkSongSlot = (t, song) =>
  ({id:uid(), kind:'tanda', tandaId:t.id, count:1, role:null, songIds:[song.id]});

/** 분위기·세트를 따지지 않고 want 곡을 무작위로 뽑는다. 코르티나는 넣지 않는다. */
function buildRandomProgram(mode, want){
  const pool = shuffled(songPool(RANDOM_MODES[mode]));
  const slots = [], seenId = {}, seenTitle = {}, spare = [];
  for(const it of pool){
    if(slots.length >= want) break;
    if(seenId[it.song.id]) continue;
    const k = songKey(it.song).toLowerCase();
    if(seenTitle[k]){ spare.push(it); continue; }   // 같은 곡명은 일단 미뤄 둔다
    seenId[it.song.id] = 1; seenTitle[k] = 1;
    slots.push(mkSongSlot(it.tanda, it.song));
  }
  // 곡이 모자라면 다른 악단이 연주한 같은 곡명으로 채운다
...
function buildProgram(mode, sets, cortinaSec, songCount){
  if(isRandomMode(mode)) return buildRandomProgram(mode, songCount);
...
function init(){
  ...
  const countSel = document.getElementById('count-input');
  const setsSel = document.getElementById('sets-input');
  const secSel = document.getElementById('cortina-seconds-input');

  const updateModeVisibility = () => {
    const isRnd = isRandomMode(modeSel ? modeSel.value : '');
    if(countSel) countSel.hidden = !isRnd;
    if(setsSel) setsSel.hidden = isRnd;
    if(secSel) secSel.hidden = isRnd;
  };
  if(modeSel){
  ...
  document.getElementById('generate-btn').addEventListener('click', () => {
    ...
    const sets = parseInt(setsSel ? setsSel.value : '1', 10) || 1;
    const sec  = parseInt(secSel ? secSel.value : '0', 10) || 0;
    const songCount = parseInt(countSel ? countSel.value : '30', 10) || 30;
    settings.cortinaSeconds = sec; lsSet(LS.settings, settings);
    programSlots = buildProgram(mode, sets, sec, songCount);
    pos = null;
    renderProgram();
  });
